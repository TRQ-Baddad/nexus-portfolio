            ...
            case 1:
                return (
                    <div className="space-y-4">
                        ...
                        <select value={trigger} onChange={e => setTrigger(e.target.value)} className="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm">
                            <option>User Signs Up</option>
                            <option>User Adds a Wallet</option>
                            <option>Portfolio Value Crosses Threshold</option>
                        </select>
                    </div>
                );
            case 2:
                        ...
                        <div className="space-y-2">
                           {conditions.map((cond) => (
                               <div key={cond.id} className="grid grid-cols-4 gap-2 items-center">
                                   <select value={cond.field} onChange={e => handleUpdateCondition(cond.id, 'field', e.target.value)} className="col-span-1 bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm">
                                       <option>plan</option>
                                       <option>totalValue</option>
                                       <option>walletCount</option>
                                   </select>
                                    <select value={cond.operator} onChange={e => handleUpdateCondition(cond.id, 'operator', e.target.value)} className="col-span-1 bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm">
                                       <option>is</option>
                                       <option>is not</option>
                                       <option>greater than</option>
                                       <option>less than</option>
                                   </select>
                                   <input type="text" value={cond.value} onChange={e => handleUpdateCondition(cond.id, 'value', e.target.value)} placeholder="Value" className="col-span-1 bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm"/>
            ...
            case 3:
                 return (
                    <div className="space-y-4">
                        ...
                        <select value={action} onChange={e => setAction(e.target.value)} className="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm">
                            <option>Send Notification</option>
                            <option>Add User to Segment</option>
                            <option>Apply Tag</option>
                        </select>
                         <textarea value={actionDetail} onChange={e => setActionDetail(e.target.value)} placeholder="Action details (e.g., notification message, tag name)..." className="mt-2 block w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm" rows={3}></textarea>
                    </div>
                );
            case 4:
                 return (
                    <div className="space-y-4">
                        ...
                        <input id="auto-name" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Onboard High Value Users" className="mt-1 block w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-md py-2 px-3 sm:text-sm" />
